...
\begin{document}
	...
	\begin{itemize}
		...
		\item in this kind of classification, the \emph{positive} class is the one we are more interested classify correctly, in this case the \texttt{bad} creditors. By default, the \emph{positive} class has label $1$, and the other, the \emph{negative}, has label $0$; thus, the class labels has been inverted.
	\end{itemize}
...
\underset{U\in \mathbb{R}^{n,d}}{\mathrm{argmin}}\, \sum_{i}^{m}\norm{\va{x}_i - UU^T\va{x}_i}^2_2, \quad \mathrm{subject\ to\ } U^TU=\mathbb{I}
	...
	\begin{enumerate}
		...
		\item for the same reason, the dataset balancing technique is applied only to the $9$ fold used for training;
