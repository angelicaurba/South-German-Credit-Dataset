...
\usepackage[bottom=0.7in, left=1in, right=1in]{geometry}
...
\begin{document}
	...
	-per navigarlo usare il link xx di jupyternotebook
...
\underset{U\in \mathbb{R}^{n,d}}{\mathrm{argmin}}\, \sum_{i}^{m}\norm{\va{x}_i - UU^T\va{x}_i}^2_2, \quad \mathrm{subject\ to\ } U^TU=\mathbb{I}
	...
	\item the \textbf{maximum variance} formulation, that aims to find as principal components the directions where the variance of the projection of data points is maximized, since the idea is that in the directions where the variance is maximum, are the ones where also the information carried is maximal. Those directions are found in the following way: given the matrix of data points $X \in \mathbb{R}^{m,d}$, and given the $d$ random variables $\va{z}_1,\va{z}_2,...,\va{z}_d$ from which the columns of $X$ has been sampled, the first principal component $\va{u}_1 = (\alpha_1, \alpha_2,...,\alpha_d)$, $\alpha_i \in \mathbb{R}$ is the one that satisfies (\ref{first_pc}).
	
	\begin{minipage}[c]{0.45\textwidth}
		\begin{equation} \label{first_pc}
			\begin{cases}
				\underset{\va{u}_1}{\mathrm{argmax}}\, \mathrm{Var}(\sum_{i}^{d}\alpha_iz_i)\\
				\sum_{i}^{d}\alpha_i^2 = 1
			\end{cases}
		\end{equation}
	\end{minipage}
	\begin{minipage}[c]{0.45\textwidth}
		\begin{equation} \label{second_pc}
			\begin{cases}
				\underset{\va{u}_2}{\mathrm{argmax}}\, \mathrm{Var}(\sum_{i}^{d}\alpha_iz_i)\\
				\sum_{i}^{d}\alpha_i^2 = 1\\
				\langle\va{u}_1, \va{u}_2\rangle = 0
			\end{cases}
		\end{equation}
	\end{minipage}

	Then, once found, the second principal component is the vector in the second best direction in terms of variance, and that is orthogonal with respect to the first one. Thus $\va{u}_2 = (\alpha_1, \alpha_2,...,\alpha_d)$ must satisfy (\ref{second_pc}), and so on until the $n$ principal components are found, each one satisfying the constraints of equation \ref{first_pc} and the constraint of being orthogonal to the previous principal components. \\ By using Lagrange multipliers, this turns out finding $\va{u}_1,\va{u}_2,...,\va{u}_n$ by solving 
	\begin{equation}
		S\va{u_i} = \lambda_1\va{u_1}, \quad \mathrm{subject\ to\ }\norm{\va{u}_i} = 1 \ \mathrm{and\ } \va{u}_i \perp \va{u}_1,...,\va{u}{i-1}\\
	\end{equation}
	Where $S$ is again the sample covariance matrix (equation \ref{scatter_matrix}), that means that $\va{u}_1,\va{u}_2,...,\va{u}_n$ are the eigenvectors of the matrix $S$, corresponding to the eigenvalues $\lambda_1, lambda_2, ...,\lambda_n$. In addition, since the variance of the points projected along $\va{u}_i$ can be rewritten as $\va{u}_i^TS\va{u}_i$, each eigenvalue of its corresponding principal component is exactly equal to the variance of the dataset along that direction, and, in particular, the first $n$ eigenvectors found with the maximization variance method, are the ones corresponding to the $n$ maximum eigenvalues of the matrix $S$. Thus, the two methods are exactly equivalent.
	\end{itemize}
	In order to decide how many principal components to take, a trade off must be found in order to take neither too little information, nor too many features. 
	\begin{figure}[h]
		\centering
		...
		\label{fig:11}
	\end{figure}
	Here [Figure \ref{fig:11}] both the explained variance (the eigenvalues drown in descending order) and the cumulative explained variance (for each eigenvalue, its ratio w.r.t. the sum of all eigenvalues is computed, then drown in a cumulative way). In this case, since there is an elbow at $15$ principal components, and since at that point the cumulative explained variance is sightly higher that $85\%$, I decided to take $15$ as number of principal component to keep.
	% TODO scree plot
	% TODO ha bisogno di normalizzazione perchè si basa sulla varianza, il centering lo fa l'algoritmo, ma controllare!!
	\subsection{KPCA} 	% TODO evaluate KPCA
	\subsection{mRMR}
