...
\underset{U\in \mathbb{R}^{n,d}}{\mathrm{argmin}}\, \sum_{i}^{m}\norm{\va{x}_i - UU^T\va{x}_i}^2_2, \quad \mathrm{subject\ to\ } U^TU=\mathbb{I}
	...
	A possible variation of the above technique is its Kernel extension. In fact, PCA tries to reduce the dimensionality by finding a linear mapping to project the data from the original space to a lower dimensional space, while the Kernel PCA does the same but it operates in a very high dimensional space (DA SISTEMARE)\\
	In particular, in the implementation by \texttt{sklearn}\footnote{For the documentation see: \url{https://scikit-learn.org/stable/modules/generated/sklearn.decomposition.KernelPCA.html}}, there are four possible kernels: poly, rbf, sigmoid and cosine.
	\begin{figure}[h]
		\centering
